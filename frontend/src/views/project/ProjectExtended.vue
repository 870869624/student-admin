<template>
  <a-card title="项目列表" style="max-width: 1200px; margin: 5px auto">
    ...
    <a-table :data-source="projects" :pagination="false" row-key="id" bordered size="middle" :loading="loading">
      ...
      <a-table-column title="延期日期" data-index="requested_extension_date" key="requested_extension_date">
        <template #default="{ text }">{{ formatDate(text) }}</template>
      </a-table-column>
      <a-table-column title="延期结束日期" data-index="approval_date" key="approval_date">
        <template #default="{ text }">{{ formatDate(text) }}</template>
      </a-table-column>
      <a-table-column title="操作" key="action">
        <template #default="{ record }">
          <a-button type="primary" @click="handleUpdateStatus(record)">项目终止
          </a-button>
